...
var ASYNC_RUNNER = require('debug')('ASYNC_RUNNER');
(function(scope) {
	...
	var AsyncRunner = function(o) {
		...
		return {
			 ...
			 */
			addTask: function(_task, _data, _run) {
				...
				if (this.getStatus() == states[INACTIVE] || this.getStatus() == states[FINISHED]) {
					...
					ASYNC_RUNNER("TASK COUNT:" + stack.length);
					taskCounter = taskCounter + 1;
					if (options.continuous) {
						ASYNC_RUNNER("RE-ACTIVATING...");
						this.executeAll();
					}
					return this;
				} else {
			...
			executeAll: function() {
				if (this.getStatus() == states[INACTIVE] || this.getStatus() == states[FINISHED]) {
					...
					if (options.logging) {
						ASYNC_RUNNER("EXECUTE ALL TASKS");
					}
					this.next();
				}
			},
			...
			next: function() {
				...
				if (options.delay > 0) {
					if (options.logging === true) {
						ASYNC_RUNNER("EXECUTION DELAY OF " + t.millisecondsToStr(options.delay));
					}
